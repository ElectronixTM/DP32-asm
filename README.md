...
```
pip install dp32_asm-0.3.1-py3-none-any.whl
```
...
Чтобы сгенерировать отладочный файл (по умолчанию dbg.json), достаточно передать флаг -d. Чтобы указать, в какой файл записать отладочную информацию - можно передать параметр --debug-file:
...
Ключевое слово `rel` указывает на то, что в этом конкретном месте нужно вставить не абсолютный адрес метки, а смещение до нее (оно может быть как положительное, так и отрицательное). Даже если смещение мало, оно все еще считается **32 битным числом**

Следует заметить, что `rel` будет встречаться почти в каждой операции ветвления, так как, согласно ее спецификации, число, стоящее после условия перехода, будет _прибавлено_ к значению счетчика команд pc. Это обязывает программиста писать `rel` перед каждой меткой для перехода, чтобы адрес до нее считался корректно. Однако есть и другие способы. Например можно использовать `branch {i=0} [r100 + label]` также перекинет вас на нужную метку со смещением, которое содержится в r100. В примере соблюдается договоренность, что этот регистр кроме нуля ничего содержать не будет, но это лишь условность для удобства написания.
...
Поддержка ошибок сделана не очень приятно для пользователя, потому что ассемблер не считает своим долгом выдавать диагностику неисправности. В основном он просто укажет на то место, с которого у него что-то пошло не так. В связи с этим для упрощения отладки далее приводятся некоторые пояснения к ошибкам.
...
Поскольку парсер не знает, где он находится в контексте исходного кода и получает только выход лексера (токены), он только укажет вам строку с символом, на котором возникла ошибка и выдаст его тип.
...
Когда вы пытаетесь положить в 8 битное поле число, которое туда поместиться не может (а могут только -128 -> 127), диагностика не скажет, что произошло переполнение, будет выдано сообщение, что типы операндов не сходятся. Дело в том, что ассемблер изо всех сил старается сделать то, что написано в коде и если программист пытается в 8 битное поле положить 277, то он попытается привести это к числовому типу процессора, которых всего 2 - I8 и I32. Поскольку в I8 положить 277 невозможно, ассемблер предположит, что это 32-битное число. Но для арифметики например 32-битное поле не задействуется нигде, поэтому ошибка скажет именно о типах операндов, а не о переполнении.
